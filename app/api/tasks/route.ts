...
export async function GET(req: NextRequest) {
  ...
  let tasks;
  // Batch 03/04/07: lead-, opportunity- or project-scoped tasks -- any
  // authenticated staff with access can see tasks tied to a specific record.
  if (leadId) {
    ...
    tasks = await sql`SELECT t.*, st.name AS staff_name FROM staff_tasks t LEFT JOIN staff st ON st.id=t.staff_id WHERE t.project_id=${projectId} ORDER BY t.created_at DESC`;
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // Lead-, opportunity- or project-scoped tasks: any signed-in staff with
    ...
    if (b.leadId || b.opportunityId || b.projectId) {
      ...
      const rows = await sql`INSERT INTO staff_tasks (staff_id, lead_id, opportunity_id, project_id, title, description, status, due_date, priority)
        VALUES (${staffId}, ${b.leadId || null}, ${b.opportunityId || null}, ${b.projectId || null}, ${b.title.trim()}, ${b.description || ""}, ${b.status || "todo"}, ${b.due_date || null}, ${b.priority || "medium"}) RETURNING id`;
